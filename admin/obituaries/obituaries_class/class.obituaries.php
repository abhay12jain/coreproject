<?php class Obituaries extends Database {
	...
	function addObituaries($argPostData){
		if($argPostData['ObituariesLocation']){
			$location=$argPostData['ObituariesLocation'];
		}
		else{
			$location='';
		}
		...
		if (array_key_exists($fileType, $image_type_identifiers)) {
			...
			if(move_uploaded_file($fileTmpPath, $dest_path))
			{
			  $arrCols = array ('ObituariesName'=>$argPostData['Obituaries_name'],
						...
						'ObituariesLocation'=>$location,
						'ObtituariesDate'=>$argPostData['ObituariesDate'],
			...
			echo $message = 'File type is not valid.';
			return $message;die();
		}
	}
    else{
...
'=>date("Y-m-d H:i:s"),
	...
	function getObituariesDetails($arrCols='',$where='',$orderBy='',$limit = ''){
		$arrCols = array('arrColumns'=>'*');
        $where='1';
        $orderBy='ObtituariesDate DESC'; 
        $limit=$limit;
        $arr_result=array();
		$arr_result=$this->select(TABLE_Obituaries,$arrCols,$where,$orderBy,$limit);
     return $arr_result;
		...
		if (array_key_exists($fileType, $image_type_identifiers)) {
			...
			if(move_uploaded_file($fileTmpPath, $dest_path))
			{
			  $arrCols = array ('ObituariesName'=>$_POST['Obituaries_name'],
						...
						'ObituariesLocation'=>$_POST['ObituariesLocation'],
						'ObtituariesDate'=>$_POST['ObituariesDate'],
						'ObituariesFileName'=>$newFileName,
						'ModifiedDate'=>date("Y-m-d H:i:s")
					);
		     $where="ObituariesId=$catid";
